...
## 5-15 試做白邊留白框，玻璃面板改為佔圖片高度至少 40%（2026-08-19，實驗性，待 Terris 確認效果）

Terris 想試試看「留白邊會不會增加設計感」，同時覺得人物照片仍然太大，並希望文字玻璃面板的高度
大約佔照片的 40%。這輪改動把「照片滿版鋪整張卡」拆成「卡片本身是白底 mat，照片內縮在裡面」兩層：

- `.jp-about_leader-card` 新增 `padding: 1.5rem`（四邊），`background-color` 從 `#eef0f3` 改成 `#ffffff` —
  這段 padding 露出的白色就是留白邊，也順帶讓照片的可視面積整體縮小一輪（不是裁切，是縮小整個照片區塊）。
- 新增 `.jp-about_leader-frame`：包住照片與玻璃疊層的內層容器，`overflow:hidden`、自己一組較小的圓角（`1rem`），
  在卡片留白之內再裁出一個「相框」，`jp-about_leader-photo` 與 `jp-about_leader-overlay` 都搬進這層裡面。
- `.jp-about_leader-overlay` 原本是 `inset:0` 撐滿整個相框高度（文字疊在滿版照片上任意位置，由 flex 內部對齊控制）；
  改成拿掉 `top`、加上 `min-height: 40%` —— 疊層現在至少佔相框高度的 40%（從底部往上量），
  但若某位主管的簡介文字比較長，仍可以自動往上長高，不會被裁掉。故 40% 是下限，不是死板的固定值。

⚠️ 這輪是 Terris 明確說「幫我試試看」的實驗性調整，不是定案：
1. 白邊寬度（`1.5rem`）、內層圓角（`1rem`）都是抓的估值，好不好看要 Terris 在 Designer 看過才知道。
...
## 5-17 移除白邊回到滿版、整卡縮小 40%、玻璃面板固定高度、文字簡化（2026-08-19）

Terris 這輪決定：白邊實驗不要了，改回滿版照片；玻璃面板高度要四卡一致（不能因簡介長短跑掉，
寧可留白也要固定）；姓名移除英文/Ph.D，職稱移除括號英文縮寫，職稱顏色改黑色；
人物照片整體再縮小 40%。

- **撤掉 5-15/5-16 的白邊與玻璃邊實驗**：`jp-about_leader-frame` 這層拆掉（`photo`、`overlay` 移回直接掛在
  `jp-about_leader-card` 下），`.jp-about_leader-card` 移除 `padding`、`background-image`、`backdrop-filter`、
  `box-shadow`，改回單純的 `background-color:#eef0f3`（僅作為圖片載入失敗時的保底色，正常狀態完全被照片蓋住）。
  `.jp-about_leader-frame` 這個 class 也直接刪除，沒有殘留未使用的樣式。
- **整卡縮小 40%**：`.jp-about_leader-card` 新增 `width:60%; margin-left:auto; margin-right:auto`，
  在維持 2 欄網格（2 上 2 下）不變的前提下，讓每張卡（含滿版照片）在自己的格子裡整體縮小、置中，
  卡片四周露出的是頁面底色，不是卡片自己的白邊。
- **玻璃面板固定高度**：`.jp-about_leader-overlay` 從 `min-height:55%`（下限、依內容可以更高）
  改成 `height:55%`（固定值，四張卡完全一致，簡介短的卡會在面板裡留白，不會面板變矮）。
- **文字簡化**：四位主管的姓名移除英文名與 Ph.D. 等學位標示（例：「康仕仲（Jessy Kang, Ph.D.）」→「康仕仲」），
  職稱移除括號內的英文縮寫（例：「創辦人暨執行長（CEO）」→「創辦人暨執行長」）。
  已用 element_id 逐一查詢 String 子節點確認四位姓名、職稱字元正確（含容易錯字的「泓」「仕」等字）。
- **職稱改黑色**：`.jp-about_leader-title` 的 `color` 從原本綁定的品牌色變數改成固定 `#000000`。
...
## 5-19 玻璃面板統一大小並頂齊、職稱英文縮寫還原、簡介文字加深（2026-08-19）

Terris 看了截圖後回饋：四張玻璃卡大小不一致、沒有對齊頂端；職稱的英文縮寫要保留；
簡介的灰字要再黑一點。

- **玻璃面板統一大小＋頂齊**：根因是 `.jp-about_leader-glass` 原本只有 `width:100%`（高度隨內容自動長高），
  靠父層 `.jp-about_leader-overlay` 的 `align-items:flex-end` 把面板貼齊底部——簡介字數不同，
  面板高度就不同，面板頂端也就高低不齊。改成 `.jp-about_leader-glass` 也加上 `height:100%`
  （填滿父層 overlay 已經固定的 `height:55%`），四張卡的玻璃面板現在物理上完全同一個尺寸，
  頂端自然對齊。內容一律從面板頂端往下排（一般 block 排列），另外加了 `overflow:hidden`
  避免簡介字數特別多時溢出面板邊界。
- **職稱英文縮寫還原**：上一輪（5-17）把職稱括號內的英文縮寫拿掉了，這輪 Terris 說要留著，
  四個職稱都改回「⋯⋯（CEO/CRO/CTO/CDO）」的格式（姓名的英文名與 Ph.D. 維持移除，不受影響）。
  已用 element_id 逐一查詢 String 節點確認四段文字正確。
- **簡介文字加深**：`.jp-about_leader-bio` 的 `color` 從原本綁定的（偏淡）品牌灰階變數，
  改成固定的 `#222222`，跟玻璃面板的對比會更明顯。

⚠️ 面板固定 `height:100%`（=overlay 的 55%）之後，如果之後有人簡介特別長，可能會被 `overflow:hidden`
裁掉尾端——目前四位主管的簡介長度都在安全範圍內，之後如果要換人或改文案，需要留意這個上限。

## 5-20 放大版面與間距、縮小玻璃面板、加入階梯式排版，並用 element_snapshot_tool 自行截圖驗證（2026-08-19）

Terris 這輪要求：左右兩張卡片間距再放大一點、整體卡片可以更大、且排版改成「稍微階梯方式」
（不要死板的水平齊平網格）。同時 5-19 遺留的問題（玻璃字卡佔太多版面、下方留白過多）也在這輪一起處理。

- `.jp-about_leaders-grid`：`width` 從 `60% → 78%`（整體卡片變大），
  `grid-column-gap` 從 `0.5rem → 1.75rem`（左右間距放大），`grid-row-gap` 從 `1rem → 3rem`
  （配合階梯位移預留足夠垂直空間，避免視覺重疊）。
- `.jp-about_leader-overlay`：`height` 從 `55% → 40%`，玻璃面板佔卡片比例明顯縮小，露出更多照片。
- 新增階梯排版：`.jp-about_leader-card` 在 `nth-child(even)` 偽類下加 `transform: translateY(3rem)`，
  讓網格右欄（第 2、4 張卡）整體往下位移 3rem，形成左右錯落的階梯視覺，
  且用 `transform` 而非 `margin-top`，不會影響 grid 本身的列高計算、不會把左欄的下一列卡片一起往下擠。

**這輪第一次用 `element_snapshot_tool` 直接對 `jp-about_leaders-grid` 元素截圖自行檢查**（而不是純憑數值猜測），
確認：四張卡文字都沒有被裁到、階梯位移沒有造成視覺重疊、玻璃面板縮小後留白明顯減少。
之後遇到類似「這樣改對不對」的視覺調整，應該優先用這個工具自行截圖驗證，
而不是改完數值就直接回報給 Terris 讓他自己去 Designer 看。

## 5-21 姓名/職稱字級再放大，職稱英文縮寫拿掉括弧（2026-08-19）

- `.jp-about_leader-name` `font-size` `1.25rem → 1.5rem`
- `.jp-about_leader-title` `font-size` `1.125rem`（原 `0.9375rem`）
- 四個職稱的英文縮寫拿掉全形括弧，改成中英文之間留一個半角空格：
  「創辦人暨執行長（CEO）」→「創辦人暨執行長 CEO」，其餘三個職稱同樣處理。

已用 `element_snapshot_tool` 直接截圖確認：字級放大後四張卡文字都沒有被固定高度的玻璃面板裁到。

## 5-22 修正日本市場詢問表單的標籤換行問題，改用 /contact 頁的無標籤設計（2026-08-19）

Terris 回饋表單的標籤文字換行變成三行（如「公司」/「名稱」/「*」疊在一起），且要求設計參考 `/contact` 頁。

**根因**：當初建這個表單時，把 label 元素的樣式掛成 `jp-about_profile-label`——這是公司資訊表格
（公司名稱/代表人⋯⋯）左側窄欄標籤用的 class，欄寬只夠放 2-4 個字。表單欄位的標籤文字較長
（如「電話（選填）」），在這個窄欄裡逐字換行，變成畫面上看到的三行堆疊。

**核對 `/contact` 頁的真實設計**：直接用瀏覽器打開已發布的 `/contact` 頁面、抓 DOM，發現
`/contact` 的表單完全沒有獨立的標籤元素——每個 `FormTextInput`/`FormTextarea` 只設定 `name`（如
「公司名稱」「電話號碼」），Webflow 會自動把這個 `name` 值渲染成輸入框的 `placeholder`，畫面上看到
的就是輸入線內的淺色提示字，沒有另外疊加的標籤文字。

**修法**：對齊 `/contact` 的做法——
- 移除本頁表單 5 個 `jp-about_profile-label` 標籤元素（公司名稱/姓名/電子郵件/電話/詢問內容）。
- 電話欄位原本用標籤文字補充「（選填）」的提示，標籤拿掉後改把該欄位的 `name` 設定
  從「電話」改成「電話（選填）」，讓自動產生的 placeholder 直接帶出「選填」提示，不用額外元素。
- 其餘欄位（公司名稱、姓名、電子郵件、詢問內容）的 `name` 本來就是正確的中文欄位名，
  拿掉標籤後會自動變成一行式 placeholder，不會再有換行問題。
