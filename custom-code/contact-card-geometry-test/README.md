...
| | 卡片 | 頭像框 | 頭像佔卡片高 |
|---|---|---|---|
| 業務卡 | 384 × 605.6 | 384 × **441.6**（1:1.15） | 72.9% |
| 居服卡（修好前） | 384 × 718 | 384 × **576**（1:**1.5**） | 80.2% |
| 居服卡（修好後） | 384 × 583.6 | 384 × **441.6**（1:1.15） | 75.7% |

> 業務卡的數字是 2026-09-10 移除「服務對象：住宿型機構・日照中心」標籤**之後**量的。
> 移除前業務卡是 384 × 659.6（頭像佔 66.9%）。
...
修好後兩張卡的**頭像尺寸完全一致**（384×441.6），卡片總高只差 22px：
業務卡的資訊區 165.6px（姓名列＋區域標籤＋Email／電話／LINE），
居服卡 142px（職稱＋一顆 CTA）。頭像佔比 72.9% 對 75.7%。

這 22px 是資訊量的差異，不是幾何設定的差異。若之後要連卡片外框都完全一樣高，
可以在 `.homecare-contact_card` 加 `min-height: 37.85rem`（= 業務卡實測的 605.6px），
資訊區的 `flex-grow: 1` ＋ `justify-content: space-between` 會把 CTA 壓到卡片底部
——但兩張卡片分屬 switcher 的兩個面板、**永遠不會同時出現在畫面上**，
差 22px 看不出來，所以先不做。
