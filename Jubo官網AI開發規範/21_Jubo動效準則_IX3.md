...
## §23 直接刪除、不做替代：Header 153、Header 106、Navbar 3（2026-09-23）

Terris：「我們應該拆掉所有 IX2」。以下三組確認「刪除後的靜止狀態＝原本動畫的起點或終點，版面完整」後直接刪除，精緻效果留到 IX3 全站重新製作時統一設計。

| 效果 | 元素 | 刪除後 | 判斷依據 |
|---|---|---|---|
| Header 153 images Slide In/Out | 7 頁頁首照片（照護助理 APP、照護推車、智齡安心寶、HI居服 APP、Amy 透鏡、JP About） | 照片停在 Slide In 的終點（x/y = 0） | Slide In 的終點就是 class 本身的位置；手機版照片本來就不顯示 |
| Header 106 Scroll | 4 頁 hero（三個解決方案頁、招募） | 照片停在 scrub 起點（keyframe 0 全為 0） | 起點＝class 本身的位置 |
| Navbar 3 menu Open/Close | `Jubo-Digital-Navbar` 元件 | 手機選單不再加白色遮罩 | 抽屜開關是 Webflow 原生 w-nav，不靠 IX2；原本白色遮罩反而蓋住整個頁面 |

### ⏸ 保留：Header 156（智齡數位頁首，3 個觸發器）

**不能直接刪**：`.header156_text-wrapper`（標語「縮短 AI 技術與場域實踐的最後一哩路，做你最值得信賴的科技夥伴」）的 class 樣式本身是 `opacity: 0`，完全靠 IX2 scrub 在 keyframe 35→40 顯示。刪掉觸發器，這句話會永久看不到。且標語 `position: absolute` 疊在照片區上，不能單純改成常駐顯示。

→ 列為 IX3 重新製作的第一個樣板（捲動敘事：照片縮小散開 → 標語浮現）。

### staging 驗證

| 項目 | 結果 |
|---|---|
| IX2 綁定 | 25 → **3**（只剩 Header 156） |
| 10 頁頁首（桌機／手機） | 所有照片 `transform: none`、opacity 1，位置與動畫終點一致；截圖確認無跑版 |
| 可見性掃描（11 頁 × 桌機／手機） | 差異 0（Amy 頁有 `axr-*` 載入動畫時間差造成的偶發差異，三次重跑結果都不同，與本次無關） |
| 智齡數位手機選單 | 開啟／點抽屜外側關閉正常，與正式站一致 |
