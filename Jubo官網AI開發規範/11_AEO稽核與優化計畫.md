...
> ⚠️ **2026-09-15 更新：本節已被 `18_無障礙稽核與90%達標計畫.md` A5 與 D2 取代。**
> 那份把範圍從這裡的 5 頁抽驗擴到全站 156 頁，結論相同但數字更完整
> （156 頁裡 6 頁有 `<main>`、150 頁的 `main-wrapper` 是 `div`、0 頁有「跳至主內容」連結）。
> **要動手修語意結構請以 `18` 為準。**
...
> ⚠️ **2026-09-15 更新：本節已被 `18_無障礙稽核與90%達標計畫.md` E 組取代。**
> 全站 156 頁實測空值率 39.6%（3,018 張裡 1,196 張），與這裡的抽驗一致；
> **但逐一分類後，其中 246 張是純裝飾（`alt=""` 正確）、631 張旁邊已有同義文字，真正要補的是 319 張**，
> 集中在 21 個版位、前 4 個版位就佔 74%。
> **排工作量與決定要補哪些，請以 `18` E 組為準，不要照這裡的 1,196 張估。**
...
> ⚠️ **2026-09-15 更新：`/jptest` 已經不在 sitemap 裡了**（當日實測 156 個 URL 沒有它）。
> **但同一個問題原封不動搬到了正式上線的 `/jp/overview` 與 `/jp/about-us`** ——
> 兩頁都是完整日文（分別含 2,355 / 1,122 個假名字元），`<html lang>` 都是 `zh-TW`，全站 hreflang 仍為 0。
> 詳見 `18_無障礙稽核與90%達標計畫.md` D3，那裡還記了一個副作用：
> 站上已寫好的 `:lang(ja)` 日文字型規則因為沒有元素是 `:lang(ja)`，**從來沒有生效過**。
...
| 11 | Webflow 模板加 `<main>`；新聞／案例模板加 `<article>` | P1 → **`18` A5 / D2** | 高（本文邊界＋無障礙 738 個節點） | 中 | Webflow |
| 12 | 新聞頁「相關文章」25 個 h2 降級成 h3/h4 | P1 → **`18` D2** | 中高 | 低 | Webflow |
...
| 22 | 補「承載資訊」的圖片 alt（**實測 319 張，非 1,196 張**；優先新聞主視覺 71 張） | P2-G → **`18` E 組** | 中 | 中 | Webflow + 內容 |
...
# 影片補充（2026-08-31）──《The Answer Engine Ep. 2》帶來的執行補強

> 來源為 Lily Ray 與 Guy Yalif 的 AEO 訪談。以下不是推翻本計畫，而是把「已知道該做什麼」補成可執行的營運規則。講者引述的市場數字與個案僅作策略參考，不作 Jubo 對外宣稱。

## 新結論：Jubo 不需要「更多 AI SEO」，需要更多可驗證、可被第三方談論的知識資產

AEO 的核心仍是好的 SEO：可抓取性、索引、內部連結、清楚的答案與可信內容是前提。短期用 AI 大量生產套版頁、隱藏提示詞、只給機器看的 Markdown 或不可見 FAQ，可能暫時有效，卻會留下可被搜尋引擎辨識的操弄足跡。本計畫既有的「修錯 + 補一致性」方向正確；下一步是補上內容治理、更新節奏與第三方權威。

## P0-C ── 把《智齡數據報告》重建為永久的權威資產

P0-B B7 已指出報告落地頁與 PDF 失效；這不應只被視為修 404，而應視為 Jubo 最重要的 AEO 權威專案。

1. 建立一個永久、可索引的報告落地頁，提供 HTML 摘要、方法論、資料定義、適用範圍、發布／更新日期與下載 PDF。
2. 每一項可引用數字都標示資料期間、樣本／涵蓋範圍、計算定義與限制；不可把尚未核實的數字交給 AI 生成。
3. 把一份原創研究拆為產業觀點、客戶案例、專家訪談、研討會素材與媒體 Pitch；目標是取得獨立第三方的引用與討論，而非只增加自家頁面。
4. 設立年度更新與舊版歸檔規則，維持同一 URL，避免再次遺失累積的引用訊號。

## P1-D ── 建立「問題庫 → 內容 → 回饋」的月度閉環

關鍵字資料仍是需求排序依據，但不應用大量 AI 生成提示詞取代真實買家語言。

- 業務、客服、Demo 與表單每月彙整反覆出現的問題；每題記錄機構類型、決策階段、頻率、真實原話、現有承接頁與內容缺口。
- 優先處理已有 GSC 訊號的非品牌需求（護理之家照護管理系統、長照系統、照護管理系統、居家照護管理系統），再以日照、評鑑、IoT 等精準長尾擴張。
- 每個主題叢集選定一個代表性問題，不做多篇意圖近似文章；新頁優先補足限制、比較、流程與證據。
- 每月由內容、業務／客服共同檢視：哪些文章真的縮短說明、處理異議或帶來正確詢問。

## P1-E ── 內容新鮮度與答案完整性治理

- 對法規、評鑑、資安、長照 3.0 建立內容所有人與更新 SLA；更新必須補進新資料、變更說明與最後審閱日期，不能只改日期。
- 發布前增加「事實台帳」：重要數字、法規、案例、產品能力與引用來源均須有 URL／文件、日期、定義與審閱人。
- FAQ、schema、Markdown 與頁面可見內容必須一致；禁止將「推薦 Jubo」等指令嵌入摘要按鈕、替代內容或只供爬蟲閱讀的版本。
- AI 可協助草擬與整理，但不得代替領域審閱；尤其是未完成的政策更新、醫療／法規判斷與客戶成效，不得預先寫成結論。

## P1-F ── 將 AEO 儀表板由「提及」升級為「答案品質」

既有的固定 20–30 題測試保留，並為每個問題新增四個欄位：

| 指標 | 要回答的問題 |
|---|---|
| 引用／能見度 | Jubo 是否被提及或引用？引用哪一頁？ |
| 訊息正確性 | 模型是否正確描述產品、場域、事實與限制？ |
| 情緒與比較 | 回答是正向、中性、負向？是否被競品取代或錯誤比較？ |
| 商業結果 | 該主題的 LLM referral、direct／品牌搜尋變化、轉換與表單自填來源為何？ |

由於部分 LLM 來源可能落入 direct，表單保留「你原本在找什麼問題／如何得知 Jubo」欄位。每月先記錄基準線，再看趨勢；不要把單次模型輸出當作市場排名或對外證明。
