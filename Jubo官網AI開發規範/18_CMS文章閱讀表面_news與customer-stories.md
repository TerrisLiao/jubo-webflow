...
## 25. 效能稽核與手機表格修正（2026-09-22）

### 25-1. 本次改動的效能影響：量不到

同一頁、同一環境，把模板 CSS 開／關各三輪取中位數：

| 頁面 | CSS 開 | 關 | 差異 |
|---|---|---|---|
| jubostory13（6 個玻璃圖說） | 13.5 | 13.3 | +0.2 |
| jubostory11（0 個玻璃圖說） | 12.6 | 13.6 | −1.0 |
| /news 補助新制 | 13.2 | 13.2 | 0.0 |

玻璃圖說單獨 A/B：開 12.7 / 關 12.3，差 −0.4。都在雜訊範圍。
模板 CSS 大小：news 4.6KB、story 5.7KB（未壓縮）。

> 量測限制：此環境基準 FPS 僅約 13，不代表真實裝置；有效的是相對比較。

### 25-2. 站台既有的效能瓶頸：`filter: blur(50px)`

| 方案 | FPS | 相對現況 |
|---|---|---|
| 現況 `blur(50px)` / `blur(40px)` | 12.2 | 1.00x |
| 半徑降到 25px / 20px | 19.3 | 1.58x |
| 半徑降到 12px / 10px | 19.1 | 1.57x |
| **完全拿掉 filter** | **44.7** | **3.66x** |
| 加 `contain: paint` | 12.9 | 1.06x |
| 拿掉 `will-change` | 7.2 | **0.59x** |

**關鍵**：把半徑調小沒有用（25px 與 12px 給出相同數字），成本來自
「大型 fixed 元素上存在 filter」本身，每幀都要重新光柵化。
`will-change: transform` 是在幫忙的，不能拿掉。

對象（**站台既有，非本次改動**）：

| 元素 | 設定 | 尺寸 | 內部有動畫的子元素 |
|---|---|---|---|
| `.home-gradient-bg` | `blur(50px)`、`position: fixed`、`will-change: transform` | 1280×900 | **0** |
| `.gradient-bg.is-cta` | `blur(40px)`、`position: absolute` | 1280×711 | **0** |

內部沒有動畫子元素，代表模糊後的結果是靜態的——**可以預先烘成圖片**當背景，
拿掉 filter 後效能應接近「完全拿掉」那一列。但 `.home-gradient-bg` 是全站共用元件，
依 CLAUDE.md 不可未經授權修改，待決。

### 25-3. `backdrop-filter` 用量

`/news` 單頁 53 個，**其中 0 個在文章內文**（全部來自站台元件）：

| 元件 | 數量 |
|---|---|
| `.single-glass-board` | 39 |
| `.news-card_headline-wrap` | 5 |
| `.mega-menu_blur` | 3 |
| `.glass-element.is-cta` | 3 |
| `.glass-effect.is-icon` | 3 |

### 25-4. 手機表格：診斷錯誤與真正的修正

原本判斷是「寬表格可以捲但沒有提示」，加了 scroll-shadow 後實測發現
**表格根本沒有溢出**（內容 332px = 可視 332px），因為 `width: 100%` 讓它擠進畫面，
代價是中間欄被壓成「全年／累計／入住／日數」每行兩三個字。

真正的修正是給最小寬度，讓它保持可讀寬度並允許左右捲：

```css
@media screen and (max-width: 767px) {
  .richtext table { min-width: 34rem !important; }
}
```

搭配 Komarov scroll-shadow（純 CSS，捲到底時陰影自動消失）。

驗證：

| 檢查 | 結果 |
|---|---|
| 手機表格是否可捲 | **是**（內容 544px / 可視 332px） |
| 捲動陰影是否可見 | 是。右邊緣由內往外 `rgb(237,249,251)` → `225` → `215` → `207` |
| 背景層數／attachment | 4 層／`local, local, scroll, scroll` |
| 桌機是否受影響 | 否（溢出 false、`min-width` 0、背景層 0） |

### 25-5. repo 與線上一致性

貼進 Webflow 時我把註解縮短了，導致 repo 檔案與線上只有註解不同。
已改為**從線上抓回內容寫進 repo**（零轉錄），現在逐字相同：

| 檔案 | 結果 |
|---|---|
| news `<style>` | ✅ 5025 字元相同 |
| news `<script>` | ✅ 3507 字元相同 |
| story `<style>` | ✅ 6245 字元相同 |
| story 是否誤含腳本 | 否 |

往後更新模板的流程改成：改 repo → 貼進 Webflow → 發布 → **從線上抓回 repo** → 比對。

### 25-6. 其他待辦

| 項目 | 現況 | 建議 |
|---|---|---|
| CLS | 0.0639（門檻 0.1） | 文章圖片全都沒有寬高屬性，補上可壓到接近 0 |
| srcset | story 7/7 張沒有、news 2/6 張沒有 | 後台重新上傳會自動產生 |
| 過大圖片 | 原圖 1672px 顯示 768px、1538px 顯示 560px | 同上 |
| 舊文章 inline style | 仍在 CMS 內容裡，靠 `!important` 蓋著 | 後台手動清，之後可拿掉 `!important` |

---

## 26. 正式站發布（2026-09-22）

### 26-1. 更正：先前關於「未發布正式站」的說法有誤

本次工作期間我多次回報「正式站從頭到尾沒有發布過」。查證後**這是錯的**：
...
經 Terris 明確授權後發布到兩個自訂網域與 staging。

發布前檢查（staging）：

| 檢查 | 結果 |
|---|---|
| news 模板 | v8、無選到 img 的規則、min-width 與捲動陰影皆有 |
| story 模板 | v13、同上 |
| 康仕仲 4 欄表格（手機） | 可捲 544/332px、陰影 4 層 |
| 補助新制 3 欄表格（手機） | 可捲 544/332px、陰影 4 層 |
| 成大研究那篇 | HTTP 404（草稿，不會上線） |
| 圖片 | 捲動後載入失敗 0（未捲動時的「壞 2」是 lazy load 假警報） |

發布後驗證（兩個網域各測）：

| 網域 | news | story | 圖片 | 表格可捲 |
|---|---|---|---|---|
| `jubo-health.com` | v8 ✅ | v13 ✅ | 6/0 壞、7/0 壞 | ✅ |
| `www.jubo-health.com` | v8 ✅ | v13 ✅ | 6/0 壞、7/0 壞 | ✅ |

康仕仲那篇 4 欄表格在正式站手機上已可左右捲並顯示陰影；
成大研究那篇維持 404，未上線。

### 26-3. 發布 API 的注意事項

`publish_site` 的 `customDomains` 要傳**網域 ID**，不是網址字串；
傳網址會得到 `400 invalid_domain`。ID 可從 `get_site` 的 `customDomains[].id` 取得。
另外連續發布會觸發 `429 Too Many Requests`，需等待約一分鐘再重試。
...
**為什麼 Webflow 的壓縮沒有涵蓋到**：站上確實做過批次壓縮 ——
資產面板 500 張裡 453 張是 WebP、6 張 AVIF，只剩 5 張 PNG，
其中 448 張的最後更新日期都是 2026-09-07。那次壓縮是有效的。

但這 106 張文章封面**沒有一張**在資產面板裡，全部掛在
`69f82ba1d504290f910e8826` 這個 bucket（與新聞 collection 的 ID 同前綴，
應是當初從 WordPress 匯入時產生的）。Webflow 的壓縮與 `compress_assets`
只能處理站內資產，所以完全沒碰到它們。

也排除了「CDN 依 Accept 做內容協商」的可能：真實 Chrome 在真實頁面上
收到的是 `content-type: image/png`、632,226 bytes，回應沒有 `Vary` 標頭。

**教訓**：Webflow 的資產壓縮只涵蓋資產面板；從舊站匯入的 CMS 圖片在面板外，
不會被涵蓋，也查不到。日後做全站圖片瘦身時要分開盤點這兩批。
